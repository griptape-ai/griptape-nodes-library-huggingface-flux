...
class LoraSet(DataNode):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.description = "Collect multiple LoRA objects into a set for reuse across inference nodes"
        ...
        self.add_parameter(
            Parameter(
                ...
                ui_options={"display_name": "Set Name", "placeholder_text": "e.g., Portrait Style LoRAs"}
            )
        )
        ...
        self.add_parameter(
            Parameter(
                ...
                allowed_modes={ParameterMode.OUTPUT},
                ui_options={"display_name": "Status", "multiline": True, "is_full_width": True}
            )
        )
    ...
    def validate_before_node_run(self, exceptions: list[Exception] = None) -> list[Exception] | None:
        ...
        try:
            ...
            if not loras:
                exceptions.append(Exception("Please connect at least one LoRA to create a set"))
                return exceptions
                
            # Validate each LoRA dict has required fields
            for i, lora in enumerate(loras):
                if not isinstance(lora, dict):
                    exceptions.append(Exception(f"LoRA {i+1} is not a valid dict object"))
                    continue
                ...
                if 'scale' not in lora:
                    ...
                    exceptions.append(Exception(f"LoRA {i+1} 'scale' must be a number"))
    ...
    def process(self) -> None:
        ...
        try:
            ...
            global_multiplier = float(self.get_parameter_value("global_scale_multiplier"))
            ...
            for i, lora in enumerate(loras):
                if not isinstance(lora, dict) or 'path' not in lora or 'scale' not in lora:
                ...
                print(f"[LORA SET] Added LoRA {i+1}: {processed_lora.get('name', 'Unknown')} (scale: {original_scale} → {new_scale})")
            ...
            for i, lora in enumerate(processed_loras):
                ...
                type_icon = "🏠" if lora_type == "local" else "🤗" if lora_type == "huggingface" else "❓"
                status_lines.append(f"  {i+1}. {type_icon} {name} (scale: {scale:.2f})")
            ...
            # Set safe defaults
            self.parameter_output_values["lora_set"] = []
            self.parameter_output_values["status"] = f"❌ Error creating LoRA set: {str(e)}"
