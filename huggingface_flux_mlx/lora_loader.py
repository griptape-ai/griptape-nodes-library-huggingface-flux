...
class LoRALoader(DataNode):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.description = (
            "Load local LoRA .safetensors files with configurable strength"
        )
    ...
    def validate_before_node_run(
        ...
        if not file_path.suffix.lower() == ".safetensors":
            exceptions.append(
                Exception(f"File must be a .safetensors file, got: {file_path.suffix}")
            )
            return exceptions
    ...
    def process(self) -> None:
        ...
        try:
            ...
            print(
                f"[LORA LOADER] Loaded local LoRA file: {display_name} (scale: {lora_scale})"
            )
