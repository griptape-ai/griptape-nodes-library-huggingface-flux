...
class HuggingFaceModelDownload(ControlNode):
    ...
    def validate_node(self) -> list[Exception] | None:
        ...
        if not repo_id or not repo_id.strip():
            errors.append(ValueError("Repository ID is required (e.g., 'microsoft/DialoGPT-medium')"))
    ...
    def process(self) -> AsyncResult:
        ...
        def download_model() -> str:
            try:
                # Import dependencies
                from huggingface_hub import snapshot_download, ModelCard, hf_hub_download
                try:
                ...
                self.publish_update_to_parameter("download_status", f"ERROR: {error_msg}")
                    ...
                    def update(self, n=1):
                        ...
                        if current_time - self.last_update_time >= self.update_interval:
                            ...
                            if hasattr(self, 'total') and self.total and self.total > 0:
                                ...
                                # Publish the update
                                node_self.publish_update_to_parameter("download_status", progress_msg)
            ...
            try:
                ...
                self.publish_update_to_parameter("download_status", "Loading model card...")
                ...
                self.publish_update_to_parameter("model_card_metadata", model_card_metadata)
                
                # Phase 2: Download repository (slow)
                self.publish_update_to_parameter("download_status", "Starting model download...\nThis may take several minutes for large models.")
                ...
                if tqdm_available:
                    ...
                    self.publish_update_to_parameter("download_status", "📥 Downloading (progress tracking unavailable - install tqdm)...")
                    download_path = snapshot_download(
                        repo_id=repo_id,
                        token=token if token else None,
                        allow_patterns=allow_patterns
                ...
                files_list = "\n".join(sorted(downloaded_files)) if downloaded_files else "No files found"
