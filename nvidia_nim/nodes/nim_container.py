...
class NIMContainerManager(ControlNode):
    def __init__(self, **kwargs):
        ...
        self.add_parameter(
            Parameter(
                ...
                tooltip="Container lifecycle action.",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                tooltip="Container engine to use.",
            )
        )
        self.add_parameter(
            Parameter(
        ...
        self.add_parameter(
            Parameter(
                ...
                default_value="",
                allowed_modes={ParameterMode.PROPERTY},
                tooltip="Image (e.g., nvcr.io/nim/black-forest-labs/flux.1-dev).",
            )
        )
        self.add_parameter(
            Parameter(
        ...
        self.add_parameter(
            Parameter(
                name="ports",
                type="str",
                default_value="8000:8000",
                allowed_modes={ParameterMode.PROPERTY},
                tooltip="Comma-separated host:container ports (e.g. 8000:8000).",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                default_value="16g",
                allowed_modes={ParameterMode.PROPERTY},
                tooltip="--shm-size value (e.g., 16g).",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                allowed_modes={ParameterMode.PROPERTY},
                ui_options={"display_name": "NGC API Key", "secret": True},
                tooltip="NGC API key for private pulls or runtime use.",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                default_value=True,
                allowed_modes={ParameterMode.PROPERTY},
                tooltip="Pass host HF tokens (HUGGINGFACE_HUB_ACCESS_TOKEN/HF_TOKEN) into container.",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                tooltip="NIM model variant (NIM_MODEL_VARIANT).",
            )
        )
        ...
        self.add_parameter(
            Parameter(
                ...
                default_value=False,
                allowed_modes={ParameterMode.PROPERTY},
                tooltip="Enable host cache bind-mount to /opt/nim/.cache/",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                allowed_modes={ParameterMode.PROPERTY},
                tooltip="Host cache directory to bind to /opt/nim/.cache/ (set 'use_cache_mount' to True)",
                ui_options={"hide_when": {"use_cache_mount": [False]}},
            )
        )
        self.add_parameter(
            Parameter(
                ...
                default_value="{}",
                allowed_modes={ParameterMode.PROPERTY},
                tooltip="JSON object of additional environment variables.",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                default_value=True,
                allowed_modes={ParameterMode.PROPERTY},
                tooltip="Wait for /v1/health to be OK before returning.",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                default_value=300,
                allowed_modes={ParameterMode.PROPERTY},
                tooltip="Max seconds to wait for health OK.",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                allowed_modes={ParameterMode.PROPERTY},
                tooltip="Service base URL for health and outputs.",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                default_value=True,
                allowed_modes={ParameterMode.PROPERTY},
                tooltip="Run container in background (-d).",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                default_value=200,
                allowed_modes={ParameterMode.PROPERTY},
                tooltip="Tail N lines from container logs after start/status.",
            )
        )
        ...
        self.add_parameter(
            Parameter(
                ...
                ui_options={"display_name": "Container ID", "hide_property": True},
                tooltip="Container ID or last run output.",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                output_type="str",
                allowed_modes={ParameterMode.OUTPUT},
                ui_options={"display_name": "Status", "hide_property": True},
                tooltip="Human-readable status or error.",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                output_type="str",
                allowed_modes={ParameterMode.OUTPUT},
                ui_options={"display_name": "Logs", "hide_property": True},
                tooltip="Recent container logs (tail).",
            )
        )
        self.add_parameter(
            Parameter(
                ...
                tooltip="Dict with base_url, route, defaults for inference.",
            )
        )
    ...
    def _run(self):
        ...
        base_url = (
            self.get_parameter_value("base_url") or "http://localhost:8000"
        ).strip()
        ...
        if action == "start":
            ...
            if not token:
                try:
                    token = self.get_config_value(
                        service="Huggingface", value="HF_TOKEN"
                    )
                except Exception:
                    token = None
            if not token:
                token = os.getenv("HUGGINGFACE_HUB_ACCESS_TOKEN") or os.getenv(
                    "HF_TOKEN"
                )
            ...
            try:
                extra = (
                    json.loads(env_extra_raw) if isinstance(env_extra_raw, str) else {}
                )
            ...
            if use_cache_mount:
                ...
                host_path = (
                    ...
                    if cache_dir
                    else os.path.expanduser("~/.cache/nim")
                )
            ...
            # Mask token presence
            has_hf = bool(
                env_pairs.get("HF_TOKEN")
                or env_pairs.get("HUGGINGFACE_HUB_ACCESS_TOKEN")
            )
            out = (
                (out or "")
            ...
            self.parameter_output_values["logs"] = out
            cid = (
                (res.stdout or res.stderr).strip().splitlines()[-1]
            ...
            if res.returncode != 0:
                self.parameter_output_values["status"] = (
                    f"start failed: {(res.stderr or res.stdout)[:500]}"
                )
            ...
            if health_wait:
                ...
                while time.time() - t0 < health_timeout_s:
                    try:
                        r = requests.get(
                            base_url.rstrip("/") + "/v1/health/ready", timeout=5
                        )
        ...
        if action == "stop":
            ...
            self.parameter_output_values["status"] = (
                "stopped"
                if res.returncode == 0
                else f"stop failed: {(res.stderr or res.stdout)[:200]}"
            )
        ...
        if action == "remove":
            ...
            self.parameter_output_values["status"] = (
                "removed"
                if res.returncode == 0
                else f"remove failed: {(res.stderr or res.stdout)[:200]}"
            )
