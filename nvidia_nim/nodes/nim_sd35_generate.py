...
class NIMSD35Generate(ControlNode):
    _last_used_seed: int = 12345

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.category = "NIM"
        self.description = "Stable Diffusion 3.5 Large text-to-image with batching & seed control"
        ...
        self.add_parameter(Parameter(name="mode", type="str", default_value="base", allowed_modes={ParameterMode.PROPERTY}, traits={Options(choices=["base", "base+canny", "base+depth", "base+canny+depth"])}, tooltip="SD3.5 variant/mode."))
    ...
    def _run(self):
        ...
        steps = int(self.get_parameter_value("steps") or int((svc.get("defaults", {}) if isinstance(svc, dict) else {}).get("steps", 30)))
        ...
        if seed_mode == "fixed":
            ...
            actual_seed = NIMSD35Generate._last_used_seed + 1
        ...
        for i in range(loop_count):
            ...
            try:
                ...
                for idx, art in enumerate(arts[:1], start=1):
                    ...
                    filename = f"nim_sd35_{route_slug}_seed_{current_seed}_{int(time.time()*1000)}{ext}"
                    try:
