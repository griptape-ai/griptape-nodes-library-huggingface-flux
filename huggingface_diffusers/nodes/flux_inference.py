...
class FluxInference(ControlNode):
    ...
    def __init__(self, **kwargs):
        ...
        # Inputs/properties
        self.add_parameter(Parameter(name="model_config", input_types=["dict"], type="dict", allowed_modes={ParameterMode.INPUT, ParameterMode.PROPERTY}, ui_options={"display_name": "Model Config", "hide_property": True},tooltip="ModelConfig dict from selector (contains local_path)"))
        self.add_parameter(Parameter(name="prompt", input_types=["str"], type="str", allowed_modes={ParameterMode.INPUT, ParameterMode.PROPERTY}, tooltip="Prompt text"))
        self.add_parameter(Parameter(name="negative_prompt", input_types=["str"], type="str", allowed_modes={ParameterMode.INPUT, ParameterMode.PROPERTY}, tooltip="Negative prompt"))
        self.add_parameter(Parameter(name="num_inference_steps", type="int", default_value=28, allowed_modes={ParameterMode.PROPERTY}, tooltip="Sampling steps"))
        self.add_parameter(Parameter(name="guidance_scale", type="float", default_value=3.5, allowed_modes={ParameterMode.PROPERTY}, tooltip="CFG guidance scale (dev)"))
        ...
        self.add_parameter(Parameter(
            ...
            ui_options={"hide_property": True, "hide_when": {"batch_size": [1, 2, 3]}},
            allowed_modes={ParameterMode.OUTPUT},
        ))

        # Performance report for DisplayJson node
        self.add_parameter(Parameter(name="performance_report", output_type="json", allowed_modes={ParameterMode.OUTPUT}, tooltip="Timing and settings per run"))

        # Ensure initial visibility matches default batch size
        try:
            default_batch = int(self.get_parameter_value("batch_size") or 1)
            self._logger.info(f"[FluxInference] initial batch_size={default_batch}; applying visibility")
    ...
    def after_value_set(self, parameter: Parameter, value: Any) -> None:
        if parameter.name == "batch_size":
            try:
                ...
                self._logger.info(f"[FluxInference] batch_size changed -> {batch_val}; updating visibility")
    ...
    def _run(self):
        try:
            ...
            for idx in range(1, min(batch, len(images_png)) + 1):
                ...
                try:
                    model_short = (repo_id.split("/")[-1] if "/" in repo_id else repo_id).replace(".", "").replace("-", "_").lower()
                    ...
                    art = ImageArtifact(value=data, format="PNG", width=width, height=height)
