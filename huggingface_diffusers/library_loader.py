...
class Library(AdvancedNodeLibrary):
    def before_library_nodes_loaded(self, library_data=None, library=None):
        ...
        try:
            ...
            for s in settings:
                ...
                token = contents.get("HUGGINGFACE_HUB_ACCESS_TOKEN") or contents.get(
                    "HF_TOKEN"
                )
                if token:
        ...
        self._device, self._dtype, self._attn, self._attn_reason, self._attn_debug = (
            select_device_dtype_attn()
        )
    ...
    def _maybe_install_torch_from_wheel(self, library_data=None) -> None:
        ...
        try:
            ...
            for s in settings:
                contents = s.get("contents", {})
                url = contents.get("GT_TORCH_WHEEL_URL") or contents.get(
                    "TORCH_WHEEL_URL"
                )
                if url:
                    break
    ...
    def _maybe_upgrade_torch_cuda(self) -> None:
        ...
        probe = subprocess.run(
            [
                py,
                "-c",
                "import torch; import json; print(json.dumps({'cuda': bool(getattr(torch, 'cuda', None) and torch.cuda.is_available()), 'ver': getattr(torch, '__version__', 'unknown'), 'cudaver': getattr(getattr(torch, 'version', None), 'cuda', None)}))",
            ],
            capture_output=True,
            text=True,
        )
...
def select_device_dtype_attn():
    ...
    try:
        ...
        return (
            ...
            "sdpa",
            "no-torch",
            "platform=?, python=?, torch=?, cuda=?, cc=?, fa2=missing",
        )
    ...
    if getattr(_torch.backends, "mps", None) and _torch.backends.mps.is_available():
        debug = f"platform={sys.platform}, python={sys.version.split()[0]}, torch={_torch.__version__}, mps=available"
        return "mps", _torch.float16, "sdpa", "mps", debug
    debug = f"platform={sys.platform}, python={sys.version.split()[0]}, torch={_torch.__version__}, cpu=true"
